...
final class BackController extends AbstractController
{
    ...
    #[Route('/statistiques', name: 'app_bus_stat', methods: ['GET'])]
    public function stats(BusRepository $busRepository): Response
    {
        $stats = $busRepository->countByCategorie();
        ...
        foreach ($stats as $categorie => $count) {
            $chartData[] = [$categorie, (int)$count];
        }

        $percentageData = [['Categorie', 'Pourcentage']];
        foreach ($stats as $categorie => $count) {
            $percentage = ($count / $totalBuses) * 100;
            $percentageData[] = [$categorie, round($percentage, 2)];
        }

        return $this->render('back/bus/statistiques.html.twig', [
    ...
    public function bus(Request $request, EntityManagerInterface $entityManager, BusRepository $busRepository): Response
    {
        ...
        // Get statistics about bus categories
        $stats = $busRepository->createQueryBuilder('b')
            ->select('b.categorie, COUNT(b) as count')
            ->groupBy('b.categorie')
            ->getQuery()
            ->getResult();
